...
$(document).ready(function () {
  ...
  var recordList = document.querySelector('.recordList'); //2.建立array

  var arrayBmiRecord = []; //3.建立 BmiRecordData的各種類型
  ...
  function render() {
    ...
    arrayBmiRecord.forEach(function (item) {
      text += '<li class="recordBox"><ul><li class="recordBox__title">' + item.title + '</li><li class="recordBox__text">BMI:' + item.BMI + '</li><li class="recordBox__text">height：' + item.BMI + '</li><li class="recordBox__text">weight：' + item.BMI + '</li></ul></li>';
    });
    alert(text);
  ...
  function bmiCount() {
    ...
    console.log(arrayBmiRecord); // 渲染到html

    render();
  } //監聽事件表


  btnBmiSubmit.addEventListener('click', bmiCount); // btnBmiDelete.addEventListener('click',textDelete);
});
"use strict";
